...
**Examples:** `feat/skills-search`, `fix/oci-auth-fallback`, `docs/readme-install`
...
```
feat(skills): add search subcommand for installed skills

fix(oci): handle 401 fallback when credential helper is missing
...
- Return wrapped errors with context: `fmt.Errorf("resolving skill ref: %w", err)`
...
- Mock OCI registry calls and HTTP transports — do not hit real registries in tests.
